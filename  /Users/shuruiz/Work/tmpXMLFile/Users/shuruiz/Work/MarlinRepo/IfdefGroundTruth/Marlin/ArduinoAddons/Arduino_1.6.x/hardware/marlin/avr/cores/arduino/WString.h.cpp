...
public:
	...
	String( char *cstr = "");
	String( String &str);
	String( __FlashStringHelper *str);
	...
	explicit String(float, unknowntype char decimalPlaces=2);
	explicit String(double, unknowntype char decimalPlaces=2);
	~String(void);
	...
	String & operator = ( __FlashStringHelper *str);
	#ifdef __GXX_EXPERIMENTAL_CXX0X__
	...
	unknowntype char concat(float num);
	unknowntype char concat(double num);
	unknowntype char concat( __FlashStringHelper * str);
	...
	String & operator += ( __FlashStringHelper *str){concat(str); return (*this);}
	...
	friend StringSumHelper & operator + ( StringSumHelper &lhs,  __FlashStringHelper *rhs);
	...
	String substring( unknowntype int beginIndex )  { return substring(beginIndex, len); };
	String substring( unknowntype int beginIndex, unknowntype int endIndex ) ;

	// modification
	void replace(char find, char replace);
	void replace( String& find,  String& replace);
	void remove(unknowntype int index);
	void remove(unknowntype int index, unknowntype int count);
...
protected:
	...
	String & copy( __FlashStringHelper *pstr, unknowntype int length);
	#ifdef __GXX_EXPERIMENTAL_CXX0X__
