...
/* monitor functions will only be compiled when using ATmega128, due to bootblock size raints */
#if defined(__AVR_ATmega128__) || defined(__AVR_ATmega1280__)
#define MONITOR 1
#endif
...
int main(void)
{
	...
	if (! (ch &  _BV(EXTRF))) // if it's a not an al reset...
		...
		if (getch() == ' ') {
			if (flags.eeprom) {		                //Write to EEPROM one byte at a time
				...
				for(w=0;w<length.word;w++) {
					...
					EECR |= (1<<EEPE);
#else
					eeprom_write_byte(address.word,buff[w]);
#endif
					address.word++;
				}			
			}
		...
		if (getch() == ' ') {		                // Command terminator
			...
			for (w=0;w < length.word;w++) {		        // Can handle odd and even lengths okay
				if (flags.eeprom) {	                        // Byte access EEPROM read
					...
					putch(EEDR);
#else
					putch(eeprom_read_byte(address.word));
#endif
					address.word++;
				}
				else {
