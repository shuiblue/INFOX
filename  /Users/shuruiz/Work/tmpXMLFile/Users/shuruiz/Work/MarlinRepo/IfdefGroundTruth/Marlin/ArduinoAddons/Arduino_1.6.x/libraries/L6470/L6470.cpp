...
void L6470::setAcc(float acceleration){
  ...
  unknowntype long accelerationBYTES = AccCalc(acceleration);
  SetParam(ACC, accelerationBYTES);
}


void L6470::setDec(float deceleration){
  unknowntype long decelerationBYTES = DecCalc(deceleration);
  SetParam(DEC, decelerationBYTES);
}


long L6470::getPos(){
  unknowntype long position = GetParam(ABS_POS);
...
void L6470::setOverCurrent(unknowntype int ma_current){
...
void L6470::run(byte dir, float spd){
  // RUN sets the motor spinning in a direction (defined by the ants
  ...
  unknowntype long speedVal = SpdCalc(spd);
...
void L6470::Step_Clock(byte dir){
  // STEP_CLOCK puts the device in al step clocking mode. When active,
  // pin 25, STCK, becomes the step clock for the device, and steps it in
  // the direction (set by the FWD and REV ants) imposed by the call
...
void L6470::move(long n_step){
  ...
  // direction imposed by dir (FWD or REV ants may be used). The motor
...
void L6470::goTo_DIR(byte dir, long pos){
  // Same as GOTO, but with user rained rotational direction.
...
void L6470::goUntil(byte act, byte dir, unknowntype long spd){
...
unknowntype long L6470::AccCalc(float stepsPerSecPerSec){
  ...
  if( (unknowntype long) long(temp) > 0x00000FFF) return 0x00000FFF;
  else return (unknowntype long) long(temp);
}


unknowntype long L6470::DecCalc(float stepsPerSecPerSec){
...
unknowntype long L6470::MaxSpdCalc(float stepsPerSec){
  ...
  if( (unknowntype long) long(temp) > 0x000003FF) return 0x000003FF;
  else return (unknowntype long) long(temp);
}

unknowntype long L6470::MinSpdCalc(float stepsPerSec){
...
unknowntype long L6470::FSCalc(float stepsPerSec){
...
unknowntype long L6470::IntSpdCalc(float stepsPerSec){
  ...
  float temp = stepsPerSec * 4.1943;
  if( (unknowntype long) long(temp) > 0x00003FFF) return 0x00003FFF;
  else return (unknowntype long) long(temp);
}

unknowntype long L6470::SpdCalc(float stepsPerSec){
  ...
  if( (unknowntype long) long(temp) > 0x000FFFFF) return 0x000FFFFF;
  else return (unknowntype long)temp;
}

unknowntype long L6470::Param(unknowntype long value, byte bit_len){
  // Generalization of the subsections of the register read/write functionality.
  // We want the end user to just write the value without worrying about length,
  // so we pass a bit length parameter from the calling function.
  unknowntype long ret_val=0; // We'll return this to generalize this function
  ...
  // high, max it out.
  unknowntype long mask = 0xffffffff >> (32-bit_len);
...
void L6470::SetParam(byte param, unknowntype long value){
  Xfer(SET_PARAM | param);
  ParamHandler(param, value);
}

unknowntype long L6470::GetParam(byte param){
...
long L6470::convert(unknowntype long val){
...
unknowntype long L6470::ParamHandler(byte param, unknowntype long value){
  ...
  unknowntype long ret_val = 0; // This is a temp for the value to return.
  ...
  switch (param)
  {
    ...
    // A set of defined ants is provided for the user's convenience. Default
    ...
    // of ants provided for ease of use of these values.
    case STEP_MODE:
      ret_val = Xfer((byte)value);
      break;
    // ALARM_EN controls which alarms will cause the FLAG pin to fall. A set of ants
    ...
    // set of reasonably self-explanatory ants is provided, but users should refer
    ...
    case CONFIG:
      ...
      // comprehensive set of ants for masking and testing this register is provided, but
