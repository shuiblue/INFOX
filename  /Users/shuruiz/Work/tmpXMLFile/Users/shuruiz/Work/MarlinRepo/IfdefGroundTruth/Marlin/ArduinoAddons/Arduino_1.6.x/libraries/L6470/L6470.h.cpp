...
// ant definitions for overcurrent thresholds. Write these values to
...
// Configure the functionality of the al switch input
...
#define STATUS_MOT_STATUS_CONST_SPD (0x0003)<<13 // Motor at ant speed
...
class L6470{
  ...
  public:
    ...
    void setOverCurrent(unknowntype int ma_current);
    void setThresholdSpeed(float threshold);
    void setStallCurrent(float ma_current);
    
    unknowntype long ParamHandler(byte param, unknowntype long value);
    ...
    void goUntil(byte act, byte dir, unknowntype long spd);
    boolean isBusy();
    ...
    void free();
    int getStatus();
	void SetParam(byte param, unknowntype long value);

  private:
    long convert(unknowntype long val);
    unknowntype long GetParam(byte param);
    unknowntype long AccCalc(float stepsPerSecPerSec);
    unknowntype long DecCalc(float stepsPerSecPerSec);
    unknowntype long MaxSpdCalc(float stepsPerSec);
    unknowntype long MinSpdCalc(float stepsPerSec);
    unknowntype long FSCalc(float stepsPerSec);
    unknowntype long IntSpdCalc(float stepsPerSec);
    unknowntype long SpdCalc(float stepsPerSec);
    unknowntype long Param(unknowntype long value, byte bit_len);
    byte Xfer(byte data);
    int _SSPin;
};
