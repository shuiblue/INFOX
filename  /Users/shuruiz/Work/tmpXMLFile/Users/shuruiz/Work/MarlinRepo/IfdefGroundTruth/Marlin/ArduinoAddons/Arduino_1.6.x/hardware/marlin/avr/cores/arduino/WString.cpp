...
String::String( __FlashStringHelper *pstr)
{
	init();
	*this = pstr;
}
...
{
	init();
	char buf[1 + 8 * sizeof(unknowntype char)];
	utoa(value, buf, base);
	*this = buf;
}
...
{
	init();
	char buf[1 + 8 * sizeof(unknowntype int)];
	utoa(value, buf, base);
	*this = buf;
}
...
{
	init();
	char buf[1 + 8 * sizeof(unknowntype long)];
	ultoa(value, buf, base);
	*this = buf;
}

String::String(float value, unknowntype char decimalPlaces)
{
	init();
	char buf[33];
...
String::String(double value, unknowntype char decimalPlaces)
{
	init();
	char buf[33];
	*this = dtostrf(value, (decimalPlaces + 2), decimalPlaces, buf);
...
String & String::copy( __FlashStringHelper *pstr, unknowntype int length)
{
	if (!reserve(length)) {
		invalidate();
		return *this;
...
String & String::operator = ( __FlashStringHelper *pstr)
{
...
{
	char buf[1 + 3 * sizeof(unknowntype char)];
	itoa(num, buf, 10);
...
{
	char buf[1 + 3 * sizeof(unknowntype int)];
	utoa(num, buf, 10);
	return concat(buf, strlen(buf));
}
...
{
	char buf[1 + 3 * sizeof(unknowntype long)];
	ultoa(num, buf, 10);
	return concat(buf, strlen(buf));
}

unknowntype char String::concat(float num)
{
	char buf[20];
...
unknowntype char String::concat(double num)
{
	char buf[20];
	char* string = dtostrf(num, 4, 2, buf);
	return concat(string, strlen(string));
...
unknowntype char String::concat( __FlashStringHelper * str)
{
	if (!str) return 0;
	int length = strlen_P(( char *) str);
	if (length == 0) return 1;
	unknowntype int newlen = len + length;
	if (!reserve(newlen)) return 0;
	strcpy_P(buffer + len, ( char *) str);
	len = newlen;
	return 1;
}
...
StringSumHelper & operator + ( StringSumHelper &lhs,  __FlashStringHelper *rhs)
{
	StringSumHelper &a = _cast<StringSumHelper&>(lhs);
...
void String::remove(unknowntype int index){
	// Pass the biggest integer as the count. The remove method
	// below will take care of truncating it at the end of the
	// string.
	remove(index, (unknowntype int)-1);
}

void String::remove(unknowntype int index, unknowntype int count){
	if (index >= len) { return; }
	if (count <= 0) { return; }
